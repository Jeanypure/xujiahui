 ...
 */
class AssignTaskNextController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    public function actionPickMember()
    {
        ...
        $audit_member = Yii::$app->db->createCommand("
                            SELECT p.`purchaser` from `purchaser` p  WHERE  p.`role` =4
                         ")->queryAll();
        if ($model->load(Yii::$app->request->post()) ) {
            ...
            and `member2` in ( SELECT p.`purchaser` from `purchaser` p  WHERE  p.`role` =4)
            ")->queryOne();
            ...
            and `member2` in ( SELECT p.`purchaser` from `purchaser` p  WHERE  p.`role` =4)")->queryAll();
                if(!empty($preview_id)){
                    $preid = '';
                    $previewid = '';
